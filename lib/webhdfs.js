...
function WebHDFS(opts, requestParams) {
  ...
  ['user', 'host', 'port', 'path'].some(function iterate(property) {
    if (!opts.hasOwnProperty(property)) {
      throw new Error(
        util.format(
          'Unable to create WebHDFS client: missing option %s',
          property
        )
      );
    }
 ...
 * @private
 */
WebHDFS.prototype._getOperationEndpoint = function _getOperationEndpoint(
  operation,
  path,
  params
) {
...
WebHDFS.prototype._isRedirect = function _isRedirect(res) {
  return (
    [301, 307].indexOf(res.statusCode) !== -1 &&
...
/**
 * currently not used! 
 * TODO add this when redirect happens to check if the host is accessible
 * TODO allow then to set options if the redirect should be done to a special url
 * e.g. docker image for pseudo hdc redirects to a hash but should redirect to localhost
 */
WebHDFS.prototype._testHost = function(host, port, cb) {
  http
    .get(
      {
        host: host,
 ...
 * @private
 */
WebHDFS.prototype._sendRequest = function _sendRequest(
  method,
  url,
  opts,
  callback
  ...
  return request(
    ...
    function onComplete(err, res, body) {
      ...
      if (self._isError(res)) {
        ...
        return (
          callback && callback(new Error('Unexpected redirect'), res, body)
        );
      }
    }
  );
...
WebHDFS.prototype.chmod = function chmod(path, mode, callback) {
  ...
  var endpoint = this._getOperationEndpoint(
    'setpermission',
    path,
    extend({
      permission: mode
...
WebHDFS.prototype.readdir = function readdir(path, callback) {
  ...
  return this._sendRequest('GET', endpoint, function(err, res, body) {
    ...
    var files = [];
    if (
      res.body.hasOwnProperty('FileStatuses') &&
 ...
 * @returns {Object}
 */
WebHDFS.prototype.writeFile = function writeFile(
  path,
  data,
  append,
  opts,
 ...
 * @returns {Object}
 */
WebHDFS.prototype.createWriteStream = function createWriteStream(
  path,
  append,
  opts
) {
  ...
  var req = request(params, function(err, res, body) {
    ...
    if (res && self._isRedirect(res)) {
      ...
      var upload = request(
        extend(params, { url: redirectUrl.toString() }),
...
WebHDFS.prototype.createReadStream = function createReadStream(path, opts) {
  ...
  let redirectUrl = new URL(endpoint);
  redirectUrl.port = 50075;
  var download = request(extend(params, { url: redirectUrl.toString() }));
 ...
 * @returns {Object}
 */
WebHDFS.prototype.symlink = function symlink(
  src,
  dest,
  createParent,
  callback
